...
* finalize是垃圾时被自动调用的方法

### 1） 垃圾回收

* GC garbage collection

  * 在程序运行时会产生许多的内容（变量，对象）
  * 有些内容随着程序的执行就自动销毁了，比如方法栈帧
  * 但还有一些数据（对象），是在方法执行的时候产生的，但存储在堆空间中
  * 堆空间中的数据不会随着方法的弹栈（结束）而销毁
  * 堆中的数据是可以长时间保存的

  * 如果堆中的某些数据不再被使用，还依然存货，就成为垃圾了。
  * 如果垃圾过多，就会导致有用的数据无处存放，就会出现内存泄漏
...
* 垃圾对象的判定
  ...
  1. 引用计数.   对象赋值给一个变量，引用计数器+1. 将某一个变量=null，断开了引用，计数器-1
     ...
     * 此时我们已经无法使用a和b了。但a和b互相引用

  2. 根可达分析 （java）

* 什么时候垃圾回收

  1. 当存储空间满了，就会触发垃圾回收 (正常)
...
* 如何验证垃圾（对象）被回收了
  ...
  * 垃圾其实就是对象
  * 垃圾回收就是销毁对象
  * 销毁对象前，会先调用该对象的finalize方法（遗言）
    * 如果没有重写finalize，不会调用，注解销毁
    * 如果重写了finalize方法，就会调用该方法，调用后，会再进行一次垃圾检测
    * 一般不建议过多重写finalize方法，因为会增加垃圾回收的负担
  * 在finalize方法中可实现对象的复活
    * 下次垃圾回收时，就不会再调用finalize了。

# 2021-12-10

### 2） 垃圾回收算法

* 内存泄漏 与 内存溢出  ： 最终的效果就是不够用了。

1. **标记-清除**

   * 通过根可达算法，找到可达对象（不是垃圾），进行标记
   * 再检索所有对象，没有标记的（就是垃圾），被收回

   * 直观，直接操作
   * 要进行两次检索，效率较低
   * 会产生内存碎片

   <img src="images/21121001.png" style="zoom:50%;" />

2. **标记-清除-压缩**

   * 在标记清除的基础上，对空闲空间后面的对象进行一个移动（压缩），使得内存中所有对象的空间都是连续的，这样空闲空间也都是连续的，不会产生碎片

   * 因为要有对象空间的移动和引用的切换，所以要有额外的性能开销

     <img src="images/21121002.png" style="zoom:50%;" />

3. **复制算法**

   * 前提是将存储空间一分为二。每次只用其中的一半

   * 通过根可达算法，找到可达对象，直接将其复制到了另外一块内存，而且是连续摆放（不会产生碎片）

   * 然后将原来的内存空间全部清空

     <img src="images/21121003.png" style="zoom:67%;" />

* 综上所述
  * 首先每一种算法，都不是最优，既有优点，也有不足
  * 第二，无论哪种算法，在垃圾回收都是整块内存处理
  * 但实际情况是，有些对象需要频繁的检测并回收，有些对象不需要频繁的检测

4. **分代算法：**

   * 对上述3种算法的综合使用

   * 结合对象产生后被回收的几率，将堆内存空间分为 年轻代  和 老年代。比例大约1:2

   * 年轻代在垃圾回收时使用的复制算法

     * 但却将内存分成了3部分  Eden ， survivors1,survivors2    8：1：1
     * 新对象存储在Eden区
     * 经过第一次垃圾回收，将存货对象，复制到幸存区1
     * 再产生对象，依然存储在eden区。
     * 再次垃圾回收时，幸存的对象和上一次幸存的对象，会被转移到另一块幸存区中
     * 释放之前的幸存区空间，**属于复制算法**

   * 对象何时可以熬入老年代

     1.  对象每经历一轮gc，年龄就+1.当年龄达到一个阈值时，就可以进入老年代了。 最大15岁

     2. 如果新对象太大，幸存区放不下，直接被放入老年代。
     3. 新对象再往幸存区存储时，幸存区没有空间了。将幸存区对象移动至老年代
        * 老年代对象被回收的几率比较小，
        * 假设每次只被回收几个
        * 所以使用**标记-清除算法**更合适

     <img src="images/21121004.png" style="zoom:50%;" />
